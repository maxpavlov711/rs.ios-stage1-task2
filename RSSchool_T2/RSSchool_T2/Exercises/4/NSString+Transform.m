...
-(NSString*)transform {
    ...
    NSRegularExpression *alphabetRegex = [[NSRegularExpression alloc]
                                          initWithPattern:alphabetPattern options:NSRegularExpressionCaseInsensitive error:nil];
        
        
    NSString *vowelsPattern = @"[AEIOUY]";
    NSRegularExpression *vowelRegex = [[NSRegularExpression alloc]
                                       initWithPattern:vowelsPattern options:NSRegularExpressionCaseInsensitive error:nil];
    ...
    NSMutableArray* words = [[self componentsSeparatedByCharactersInSet:
        ...
        for (int i = 0; i < [word length]; i++) {
            ...
            if ([alphabetRegex firstMatchInString:ch options:0 range:NSMakeRange(0, 1)]
                && ([vowelRegex firstMatchInString:ch options:0 range:NSMakeRange(0, 1)] == nil) == !isPangram) {
                    matchesVowelsCount++;
